...
private var searchAvailableGenres: [SearchGenreItem] {
    let all: [SearchGenreItem] = [
    ...
    if KidsConfig.isKidsEdition {
        return all.filter { !KidsConfig.blockedGenreIDs.contains($0.id) }
    }
    return all
}
                    ...
                    GeometryReader { geo in
                        ...
                        VStack(spacing: 12) {
                            ...
                            Text(searchMode == .ai ? "Ask Sigma AI Anything" : "Search Movies & TV Shows")
                                ...
                                .foregroundStyle(.secondary)
                            if searchMode == .ai {
                                Text(KidsConfig.isKidsEdition ? "Try \"find funny Disney movies\" or \"best superhero cartoons\"" : "Try \"recommend a 90s action thriller\" or \"best sci-fi shows\"")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                                    .multilineTextAlignment(.center)
                                    .padding(.horizontal, 32)
                            } else {
                                Text(KidsConfig.isKidsEdition ? "Search favorite animated movies, cartoons, and family shows" : "Use the search bar or tap the ☰ filter button to browse by genre, year, actor, and ratings")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                                    .multilineTextAlignment(.center)
                                    .padding(.horizontal, 32)
        ...
        NavigationStack {
            Form {
                ...
                Section("Starring Actor / Cast") {
                    TextField(KidsConfig.isKidsEdition ? "e.g. Tom Hanks, Chris Pratt, Kristen Bell" : "e.g. Tom Cruise, Cillian Murphy", text: $draftFilters.actorName)
                }
